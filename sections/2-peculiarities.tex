...
In this chapter, we examine the peculiarities of \reacttt{useState}~(\cref{sec:useState}) and \reacttt{useEffect} (\cref{sec:useEffect}).

We introduce a simple applicative language for \Rtrace{} that captures the essence of React Hooks and use it throughout the thesis to illustrate the examples and the semantics.
...
\begin{figure}[t]
  ...
  \begin{bnf}[rccll]
    ...
    \ell :in: $\mathbb{N}$
    ;;
    P : \dom{Prog} ::= $\Overline{D}\ e$
    ;;
    D : \dom{ComDev} ::= $\cfunc{C}{x}{e}$
    ;;
    e : \dom{Exp} ::=
    | () // true // false // $n$ // $x$ // $C$
    | $e \oplus e$ // $[\Overline{e}]$ // $\print{e}$ // $\cond{e}{e}{e}$ // $\seq{e}{e}$
    | $\func{x}{e}$ // $\app{e}{e}$ // $\letbind{x}{e}{e}$
    | $\stbind{x}{e}{e}$ // $\eff{e}$
  \end{bnf}
...
\begin{itemize}
  ...
  \item The \reacttt{useEffect} Hook enables running arbitrary code~$e$ after a component is rendered.
    ...
    The expression passed to \mbox{\reacttt{useEffect},}~$e$, is called an \emph{Effect} \citep{Met25Syn}.
\end{itemize}
...
\begin{wrapfigure}[3]{l}{0.383\linewidth}
  \vspace*{-\intextsep}
  \begin{reactcode}
let Counter x =
  let s := 0 in
  [s, button (fun _ -> s := s+2)];;
Counter 0
  \end{reactcode}
\end{wrapfigure}
\noindent Consider a counter that adds its value by two when clicked.
...
\begin{wrapfigure}[4]{r}{0.36\linewidth}
...
Return
    \end{console}
    {\footnotesize\vspace{-.6\baselineskip}\hspace{2em}\reacttt{button} \faHandPointUp[regular]}
...
To understand a more peculiar aspect of \reacttt{useEffect}, consider the following \texttt{Self\-Counter} where an Effect creates an autonomous rendering cycle:
\begin{center}
