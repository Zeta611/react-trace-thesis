\chapter{Full Operational Semantics of \Rtrace}\label{app:chap:full}
...
\begin{proof}
  Suppose $\Step*<\phase{Succ}>[\pi,\sigma[x\mapsto v]]{e}{v,\pi',\omega}$ and $\Step*<\phase{Succ}>[\hat \pi,\sigma[x\mapsto v]]{e}{\hat v',\hat \pi', \hat \omega}$, for some~$\pi$ such that~$\pi.\field{spec} = \<C, v\>$ where~$\delta[C] = \<\lfun{x}{e}, \sigma\>$ and~$\hat\pi \approx \pi$.
...
\begin{proof}
  ...
  \begin{description}
    ...
    \item[{\normalfont\Rule{CheckIdle}}] ($t = p$, $m[p] = \pi$, and $\hat m[p] = \hat \pi$).
      ...
      \begin{itemize}
        ...
        \item $\dec{Check} \in \hat \pi.\field{dec}$:
          ...
          Since $\pi \approx \hat \pi$, $\sem{normalize}(\hat \pi) = \sem{normalize}(\pi)$, we have~$\hat\pi' = \pi$.
    ...
    \item[{\normalfont\Rule{CheckEffect}}] ($t = p$).
      Since $\sem{normalize}(\hat \pi) = \sem{normalize}(\pi)$ and $\dec{Check} \in \sem{normalize}(\pi).\field{dec}$, $\dec{Check} \in \hat \pi.\field{dec}$.
...
\chapter{Comparison of Reactive UI Frameworks}\label{app:sec:frameworks}
