...
\section{Extensions to Other Hooks}
While our semantics focuses on the two most prevalent Hooks \reacttt{useState} and \mbox{\reacttt{useEffect},} it's designed to be extensible.
...
\begin{description}
  ...
  \item[$\textbf{\texttt{useMemo}}^\ell$] Add $\pi.\field{memst}$ storing values and their dependencies~$[\Overline[\ell]{\ell \mapsto \{\field{val}\!\colon \!v, \field{deps}\!\colon \![\Overline{v}]\}}]$.
  ...
  \item[Custom Hooks] Supported by allowing user function definitions~(\cref{fig:syntax}) and \dom{Def\-Table}~(\cref{sec:dom}).
    No additional machinery is required as custom Hooks are compositions of primitive Hooks.
\end{description}


\section{Verifying the React Compiler}
