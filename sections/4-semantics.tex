...
\begin{figure}[t]
  \centering
    \begin{minipage}{.26\textwidth}
      \begin{reactcode}[basicstyle=\linespread{1.2}\scriptsize\ttfamily]
let Bin n =
  if n = 0 then () else
  [Bin(n-1), Bin(n-1)];;
Bin 2
...
Standard rules related to the base logic are not presented.
...
For a constant (\Rule{CommitEffsConst}) and a closure (\Rule{Commit\-Effs\-Clos}), there are no Effects to commit, so the tree memory remains unchanged.
For an array of trees (\Rule{CommitEffsArray}), we recursively commit the Effects of each child tree.
\begin{figure}[t]
  \centering
  \input{semantics/commitEffs}
...
Demo 0
    \end{reactcode}
    \end{minipage}
    \caption{An example component \texttt{Demo} showcasing the unnecessary re-rendering issue and the top-level setter issue.}\label{fig:exdemo}
  \end{subfigure}\\
  \begin{subfigure}[b]{\linewidth}
    \centering
    \smaller[1]
    ...
    \begin{minipage}[t]{0.2\linewidth}
      ...
      \end{tabular}
    \end{minipage}
    \caption{The render step transitions for \texttt{Demo 0}.}\label{fig:demo0}
  \end{subfigure}%
  \caption{An example to illustrate the operational semantics of Hooks.}\label{fig:illust}
\end{figure}
To illustrate the operational semantics of Hooks in action, we walk through the execution of the \reacttt{Demo} component in \cref{fig:exdemo}.
...
The render step transitions for |Demo 0| is illustrated in \cref{fig:demo0}.
...
In addition, the intermediate state that triggers a retry is listed below the initial configuration~$\<\texttt{Demo 0}, [\texttt{Demo} \mapsto \dots]\>$ in \cref{fig:demo0}.
\begin{figure}[t]
  \centering
\end{figure}
...
\begin{enumerate}[start=0]
  ...
  \item Now we are in rendered mode~$\rendermode$, and we commit Effects (\Rule{StepEffect}).
    \begin{enumerate}
      ...
      \item \dec{Effect} decision is removed and the decision set is now $\{\dec{Check}\}$ (\Rule{Commit\-Effs\-Path}).
    \end{enumerate}
  ...
  \item Now we are in check mode~$\checkmode$, and we check for a re-render (\Rule{StepCheck}).
    \begin{enumerate}
      ...
      \item The closure view spec is initialized as it is of different type with~$\<\>$ (\Rule{Re\-concile\-Other}).
    \end{enumerate}

  \item We are again in rendered mode~$\rendermode$, so we commit Effects (\Rule{StepEffect}).
    \begin{enumerate}
      \item Again, the view has $\dec{Effect}$ on and we commit the Effect (\Rule{Commit\-Effs\-Path}), but \reacttt{setS} is not called this time.
